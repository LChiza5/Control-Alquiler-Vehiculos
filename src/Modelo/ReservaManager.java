...
public class ReservaManager {
  ...
  private final ReservaLista reservaLista;      // tus reservas “activas/confirmadas”
  private final ReservaQueue reservaQueue;    // lista de espera

    public ReservaManager(clienteList clienteLista, vehiculoList vehiculoLista, ReservaLista reservaLista, ReservaQueue reservaQueue) {
  ...
  /** Crear reserva buscando por TIPO. Si no hay disponible, encola (vehiculo = null, se usa tipoSolicitado). */
  public Reserva crearReservaPorTipo(String cedulaCliente, TipoVehiculo tipo,LocalDate inicio, LocalDate fin) throws ReglaDeNegocioException, EntidadNoEncontradaException {
  ...
  /** Modificar vehículo de una reserva confirmada (si no ha iniciado). */
  public void modificarVehiculoReservado(int idReserva, String placaNueva) throws ReglaDeNegocioException, EntidadNoEncontradaException {
  ...
  /** Cancelar reserva confirmada si aún no inicia. */
  public void cancelarReserva(int idReserva) throws EntidadNoEncontradaException {
    Reserva r = reservaLista.find(idReserva);
    if (r == null) throw new EntidadNoEncontradaException("Reserva no encontrada");
    if (!r.getInicio().isAfter(LocalDate.now().minusDays(1)))
  ...
  /** Intentar confirmar una reserva en espera (cola). */
  public Reserva sacarDeEsperaYConfirmar(TipoVehiculo tipo, LocalDate inicio, LocalDate fin) {
    ...
    for (int i = 0; i < size; i++) {
      Reserva r = reservaQueue.siguiente(); // sacamos el primero
      if (r == null) break;

      if (r.getTipoSolicitado() == tipo && mismasFechas(r, inicio, fin)) {
        candidata = r;
        break;
      } else {
        reservaQueue.add(r); // la devolvemos al final
      }
    }
  ...
  /** Solapamiento de rangos [a1,a2) y [b1,b2). */
